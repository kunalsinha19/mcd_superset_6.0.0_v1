...
| 6 | Dangerous Method (OPTIONS) enabled | 🟢 Fixed (commit `c5aadf4`) | `before_request` hook in `initialization/__init__.py` 404s any OPTIONS request outside `CORS_OPTIONS['resources']` (404, not 405 — a 405 must carry its own Allow header by spec, which would just recreate the same disclosure). Legitimate CORS preflight on `/api/v1/*` untouched. Verified live: `OPTIONS /superset/welcome/` and `/health` now both 404, no `Allow` header. |
| 7 | Outdated Apache Superset version | 🟢 Fixed | Unchanged from section B. |
| 8 | Password History Not Maintained | 🟢 Fixed (commit `c5aadf4`) | New `user_password_history` table + `superset/security/password_policy.py` (checks by re-hashing candidates against stored hashes, never stores plaintext; history size 3 per the audit's own recommendation). Wired into all three places a password actually gets set: self-service (`views/users/api.py`), admin-sets-another-user's (`SupersetUserApi` in `security/manager.py`), and `flask fab reset-password`/CLI (`SupersetSecurityManager.reset_password`). Verified live: resetting to a value then immediately reusing it is rejected. |
...
| 10 | Buffer overflow (missing length validation) | 🟢 Fixed (commit `c5aadf4`) | Server-side `assert_length_ok()` on email/first_name/last_name/password at all three password-setting call sites above, plus client-side `maxLength` on the login form. Verified live: a 200-char password is server-side rejected. Scope: covers the password-adjacent fields the audit's own evidence was about, not an app-wide sweep of every form. |
| 11 | Login Form Not Protected (no CAPTCHA) | 🟢 Fixed (commit `c5aadf4`) | New self-hosted math-challenge CAPTCHA (`superset/security/login_captcha.py`) — deliberately not `flask_wtf.recaptcha.RecaptchaField`, since this deployment has no real Google/hCaptcha keys and that would be a no-op, not a fix. Answer stored server-side in session, never sent to client, single-use. `SupersetAuthDBView` overrides FAB's `login()` to check it before credentials are even checked. Verified live end-to-end: correct password + wrong captcha answer is rejected (still 401 on `/api/v1/me/` after); correct password + correct answer logs in normally. |
