...
class SupersetSecurityManager(  # pylint: disable=too-many-public-methods
    ...
    def raise_for_access(  # noqa: C901
        ...
        if sql and database:
            ...
            # `query` is only ever meant to be a transient, in-memory stand-in
            # here -- expunge it in case assigning the `database` relationship
            # pulled it into the session via cascade. On SQLAlchemy 2.0 that
            # no longer happens, and expunge() on an object that was never
            # added raises InvalidRequestError instead of being a no-op, so
            # only call it if the object is actually present.
