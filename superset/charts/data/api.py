...
class ChartDataRestApi(ChartRestApi):
    ...
    def _send_chart_response(  # noqa: C901
        ...
        if result_format in ChartDataResultFormat.table_like():
            ...
            # CSV, Excel, and PDF downloads all share the same
            # "<chart name>_<timestamp>" naming convention.
            chart_name = "chart"
            dashboard_name = None
            try:
                ...
                nested = parsed.get("form_data", {})
                slice_id = nested.get("slice_id") or parsed.get("slice_id")
                if slice_id:
                    from superset.daos.chart import ChartDAO
                    chart = ChartDAO.find_by_id(slice_id)
                    if chart:
                        chart_name = chart.slice_name or "chart"
                dashboard_id = nested.get("dashboardId") or parsed.get("dashboardId")
                if dashboard_id:
            ...
            if len(result["queries"]) == 1:
                ...
                if is_csv_format:
                    return CsvResponse(
                        data, headers=generate_download_headers("csv", filename)
                    )
                if is_pdf_format:
                    import io as _io
                    ...
                    df = _pd.read_csv(_io.StringIO(data))
                    pdf_bytes = df_to_pdf(df, chart_name, dashboard_name)
                    return PdfResponse(
                        pdf_bytes, headers=generate_download_headers("pdf", filename)
                    )
            ...
            def _process_data(query_data: Any) -> Any:
                ...
                if result_format == ChartDataResultFormat.PDF:
                    ...
                    df = _pd.read_csv(_io.StringIO(query_data))
                    return df_to_pdf(df, chart_name, dashboard_name)
                return query_data
