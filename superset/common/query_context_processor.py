...
class QueryContextProcessor:
    ...
    def get_data(
        ...
        if self._query_context.result_format in ChartDataResultFormat.table_like():
            ...
            result = None
            if self._query_context.result_format in (
                ChartDataResultFormat.CSV,
                ChartDataResultFormat.PDF,
            ):
                # PDF export reuses the CSV-formatted data here; the API
                # layer (which has the chart/dashboard names) converts it
                # into an actual PDF via superset.utils.pdf.df_to_pdf.
