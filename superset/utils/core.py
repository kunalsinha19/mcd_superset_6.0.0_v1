...
def pessimistic_connection_handling(some_engine: Engine) -> None:
    ...
    def ping_connection(connection: Connection) -> None:
        # SQLAlchemy 2.0 removed connection "branching" (the old `branch`
        # parameter) along with 'connectionless'/'close with result'
        # execution, so neither applies here anymore.
        ...
        try:
            ...
            # SQLAlchemy 2.0 connections auto-begin a transaction on first
            # use, so the SELECT above leaves one open. Roll it back so the
            # connection is handed back in a clean state -- otherwise code
            # further down the line that calls connection.begin() (e.g.
            # engine.begin(), used by create_all()/migrations) fails with
            # "This connection has already initialized a SQLAlchemy
            # Transaction() object via begin()".
            connection.rollback()
