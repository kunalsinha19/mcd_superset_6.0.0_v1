...
def _patch_wtforms_json_extra_filters_incompatibility() -> None:
    """
    Work around an incompatibility between the (unmaintained) wtforms-json
    package and modern WTForms.

    wtforms_json.init() monkey-patches Field.process()/FormField.process()
    with a wrapper that only accepts `(self, formdata, data=...)`. WTForms
    3.1+ added an `extra_filters` keyword argument that Form.process() now
    always passes to field.process(), which the old wrapper doesn't accept
    -- raising `TypeError: ...process() got an unexpected keyword argument
    'extra_filters'` on every form submission (e.g. the login form).

    Re-wrap the already-patched process() methods so they accept and drop
    `extra_filters`, restoring compatibility without touching the
    third-party package.
    """
    from wtforms.fields import Field, FormField

    def tolerate_extra_filters(process: Callable[..., None]) -> Callable[..., None]:
        def wrapped(
            self: Field, formdata: Any, data: Any = _NO_DATA, **kwargs: Any
        ) -> None:
            kwargs.pop("extra_filters", None)
            if data is _NO_DATA:
                process(self, formdata, **kwargs)
            else:
                process(self, formdata, data=data, **kwargs)

        return wrapped

    Field.process = tolerate_extra_filters(Field.process)
    FormField.process = tolerate_extra_filters(FormField.process)
...
class SupersetAppInitializer:  # pylint: disable=too-many-public-methods
    ...
    def pre_init(self) -> None:
        ...
        """
        wtforms_json.init()
        _patch_wtforms_json_extra_filters_incompatibility()
    ...
    def check_and_warn_database_connection(self) -> None:
        ...
        try:
            with self.superset_app.app_context():
                # Simple connection test. `Engine.execute` was removed in
                # SQLAlchemy 2.0; use an explicit connection instead.
    ...
    def configure_fab(self) -> None:
        ...
        # SQLAlchemy 2.0 compatibility: skip FAB table creation during app init
        # Flask-Migrate (Alembic) will handle all schema creation
        from flask_appbuilder.security.sqla.manager import SecurityManager
        original_create_db = SecurityManager.create_db
        
        def skip_create_db(self):
            # Skip table creation - Flask-Migrate handles it
            pass
        
        SecurityManager.create_db = skip_create_db
        
        try:
            # Commit any pending autobegin transaction
            db.session.commit()
            db.session.close()
            db.engine.dispose()
            ...
            # Restore original create_db for future use
            SecurityManager.create_db = original_create_db
