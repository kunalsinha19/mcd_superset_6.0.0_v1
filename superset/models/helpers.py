...
class ImportExportMixin(UUIDMixin):
    # Newer SQLAlchemy versions scan class-level annotations on mapped
    # classes (and their mixins) to decide whether the "Annotated
    # Declarative Table" form is in use. The plain annotated attributes
    # below (and on subclasses, e.g. Slice.query_context_factory) are
    # regular Python/instance attributes, not ORM-mapped columns, so this
    # mixin must opt every mapped subclass out of that stricter scan.
...
class AuditMixinNullable(AuditMixin):
    ...
    @declared_attr
    def created_by_fk(self):  # pylint: disable=arguments-renamed
        return sa.Column(
            sa.Integer,
    ...
    @declared_attr
    def changed_by_fk(self):  # pylint: disable=arguments-renamed
        return sa.Column(
            sa.Integer,
            sa.ForeignKey("ab_user.id"),
            default=get_user_id,
