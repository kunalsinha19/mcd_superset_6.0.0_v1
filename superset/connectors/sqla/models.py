...
class BaseDatasource(AuditMixinNullable, ImportExportMixin):  # pylint: disable=too-many-public-methods
    ...
    @declared_attr
    def slices(self):  # noqa: ANN201
        # No return annotation: SQLAlchemy's Annotated Declarative scanner
        # requires @declared_attr return annotations to be wrapped in
        # Mapped[...]; a plain "-> RelationshipProperty" hint raises
        # MappedAnnotationError. Leaving it unannotated skips that check.
        return relationship(
            "Slice",
            overlaps="table",
