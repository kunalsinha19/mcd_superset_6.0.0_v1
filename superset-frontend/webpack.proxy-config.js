...
function processHTML(proxyResponse, response) {
  ...
  if (responseEncoding === 'gzip') {
    ...
    // Lazily required: `simple-zstd` checks for the `zstd` CLI binary at
    // import time and throws if it's missing. Only needed here, when the
    // dev-server proxy actually receives a zstd-encoded response -- not
    // for every webpack invocation (e.g. `npm run build`, which never
    // reaches this code path but was still crashing on module load).
